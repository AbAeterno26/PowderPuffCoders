...
class BreadthFirstSearch:
    ...
    def execute(self):
        ...
        self.queue = deque([(1, initial_grid)])  # Use a deque as a queue

        while self.queue:
            current_amino_id, grid = self.queue.popleft()
    ...
    def get_children(self, grid, current_amino_id):
        ...
        for direction in self.directions:
            ...
            if grid_child.is_valid(next_pos) and next_pos not in grid_child.locations:
                ...
                self.queue.append((current_amino_id + 1, grid_child))
...
#         """Folds an entire protein using breadth-first search"""
#         # Initialize the queue with the starting position (0, 0)
#         self.queue = [(1, copy.deepcopy(self.grid))]

#         while self.queue:
#             current_amino_id, grid = self.queue.pop(0)
